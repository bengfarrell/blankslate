...
/**
 * CONFIG Byte Layout (assumes report ID at byte 0)
 * 
 * This matches the Python stable config format:
 * Byte 0: Report ID (implicit in config)
 * Byte 1: Status byte (hover/contact/buttons)
 * Bytes 2-3: X coordinate (little-endian, 16-bit)
 * Bytes 4-5: Y coordinate (little-endian, 16-bit)
 * Bytes 6-7: Pressure (little-endian, 16-bit)
 * Byte 8: Tilt X (bipolar: 0=neutral, <128=left, >128=right)
 * Byte 9: Tilt Y (bipolar: 0=neutral, <128=forward, >128=backward)
 * 
 * Note: Mock generator creates packets WITHOUT report ID (status at byte 0).
 * The generateDeviceConfig function adds +1 offset when packetIncludesReportId=false.
 */
export const BYTE_LAYOUT = {
  ...
  tiltX: { byteIndex: [8] },
  tiltY: { byteIndex: [9] },
  tabletButtons: { byteIndex: [2] }, // Button data at config byte 2
} as const;

/**
 * RAW Packet Structure (without report ID, as generated by mock)
 * Use this for directly inspecting mock packet bytes.
 */
export const RAW_PACKET_LAYOUT = {
  status: { byteIndex: [0] },
  x: { byteIndex: [1, 2] },
  y: { byteIndex: [3, 4] },
...
export const STATUS_BYTES = {
  ...
  // Tablet button interface status (driverless mode)
  keyboard: 0,   // Keyboard interface sends status 0
  buttons1: 1,   // Some buttons send status 1
  buttons3: 3,   // Button 8 sends status 3
  buttons6: 6,   // Another button status byte
  
  // Tablet button status (driver mode)
  buttonsDriver: 240,  // 0xF0 - Driver mode button status
} as const;

/**
 * Complete status mapping as it should appear in the generated config
 * Includes BOTH driverless (0, 1, 3, 6) AND driver mode (240) button status bytes
 */
export const STATUS_VALUES = {
  // Driverless button status bytes
  '0': { state: 'keyboard' },
  '1': { state: 'buttons' },
  '3': { state: 'buttons' },
  '6': { state: 'buttons' },
  ...
  '192': { state: 'none' },
  // Driver mode button status byte
  '240': { state: 'buttons' },
} as const;

/**
 * All button-mode status bytes that should be in a universal config
 */
export const ALL_BUTTON_STATUS_BYTES = [0, 1, 3, 6, 240] as const;
...
/**
 * Scan code to button mapping as it should appear in the config (DRIVERLESS mode)
 * These are the scan codes sent when the XP-Pen driver is NOT installed
 */
export const TABLET_BUTTON_VALUES_DRIVERLESS = {
  '5': { button: 1 },
  '8': { button: 2 },
...
/**
 * Status overrides for buttons that share the same scan code (driverless mode)
 */
